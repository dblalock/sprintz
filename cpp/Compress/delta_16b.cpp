...
//     if (write_size) {
//         metadata_len = write_metadata_simple_16b(dest, ndims, len);
//         dest += metadata_len;
//         orig_dest = dest;
//     }
...
//     uint16_t ndims;
//     uint32_t len;
//     src += read_metadata_simple_16b(src, &ndims, &len);
//     return decode_delta_rowmajor_16b(src, len, dest, ndims);
// }
...
inline int64_t encode_doubledelta_serial(const uint16_t* src, uint32_t len,
    int16_t* dest, uint16_t lag, const uint16_t* prev_vals,
    int16_t* prev_deltas)
{
    ...
    if (own_prev_vals) {
        prev_vals = (uint_t*)calloc(lag * sizeof(uint_t), 1);
    }
    if (own_prev_deltas) {
        prev_deltas = (int_t*)calloc(lag * sizeof(int_t), 1);
    }

    // encode first row using prev_vals array passed in (or initialized to 0)
