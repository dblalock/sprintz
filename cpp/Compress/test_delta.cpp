...
TEST_CASE("delta_rowmajor_8b (no compression)", "[rowmajor][delta]") {
...
//TEST_CASE("doubledelta_rowmajor_8b serial no compress", "[rowmajor][delta][dbg]") {
...
//        CAPTURE(ndims);
//        auto comp = [ndims](uint8_t* src, size_t len, int8_t* dest) {
//            return encode_doubledelta_serial(src, (uint32_t)len, dest, ndims);
//        };
//        auto decomp = [](int8_t* src, uint8_t* dest) {
//            return decode_doubledelta_serial(src, dest);
//        };
//
//        //        TEST_SQUARES_INPUT(7, comp, decomp);
//        //        TEST_SQUARES_INPUT(256, comp, decomp);
...
TEST_CASE("delta_rle_rowmajor_8b (no compression)", "[rowmajor][delta][dbg]") {
    ...
    for (auto _ndims : ndims_list) {
        ...
        CAPTURE(ndims);
        auto comp = [ndims](uint8_t* src, size_t len, int8_t* dest) {
            return encode_delta_rle_rowmajor(src, (uint32_t)len, dest, ndims);
        };
        auto decomp = [](int8_t* src, uint8_t* dest) {
            return decode_delta_rle_rowmajor(src, dest);
        };
