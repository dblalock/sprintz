                            ...
                            uint16_t ndims, bool write_size)
{
    ...
    static const int stripe_sz = 8 / elem_sz;
    ...
    for (uint64_t g = 0; g < ngroups; g++) {
        ...
        for (int b = 0; b < group_sz_blocks; b++) { // for each block in group
            ...
            for (int16_t stripe = 0; stripe < nstripes; stripe++) {
                ...
                const uint8_t* inptr = ((const uint8_t*)src) + (stripe * stripe_sz);
...
int64_t decompress_rowmajor(const int_t* src, uint_t* dest) {
    ...
    static const uint8_t header_stripe_sz = 8;
    ...
    static const uint8_t stripe_header_sz = nbits_sz_bits * header_stripe_sz / 8;
    ...
    for (uint64_t g = 0; g < ngroups; g++) {
        ...
        *header_write_ptr = header;
        ...
        uint32_t header_pad_nstripes = DIV_ROUND_UP(ndims, header_stripe_sz);
        uint32_t header_padded_ndims = header_pad_nstripes * header_stripe_sz;
        for (uint32_t b = 0; b < group_sz_blocks; b++) {
            uint32_t src_offset = b * ndims;
            uint32_t dest_offset = b * header_padded_ndims;
            // uint32_t dest_offset = b * nstripes * header_stripe_sz;
        ...
        for (uint32_t v = 0; v < nvectors_in_group; v++) {
            ...
            if (elem_sz == 1) {
                ...
                static const __m256i fifteens = _mm256_set1_epi16(0x0f);
                __m256i header = _mm256_sub_epi16(
                    raw_header, _mm256_cmpeq_epi16(raw_header, fifteens));

                // take array of u16s and compute sums of adjacent groups
                // of 4 of them; also compute masks associated with each;
                // tricky because sad_epu8 does groups of 8, and masks are 16b
                // static const __m256i low_32b_mask = _mm256_set1_epi64x(0xffffffff);
                // __m256i even_4_entries = _mm256_and_si256(header, low_32b_mask);
                // __m256i odd_4_entries = _mm256_andnot_si256(header, low_32b_mask);

                // compute and store bitwidths
                __m256i bitwidths = _mm256_mpsadbw_epu8(
                    header, _mm256_setzero_si256(), 0);
                uint8_t* store_addr = ((uint8_t*)stripe_bitwidths) + v_offset;
                _mm256_storeu_si256((__m256i*)store_addr, bitwidths);
                ...
                _mm256_storeu_si256((__m256i*)(store_addr2 + v_offset), masks1);
            }
        }
        ...
        for (int b = 0; b < group_sz_blocks; b++) { // for each block in group
            ...
            for (int stripe = nstripes - 1; stripe >= 0; stripe--) {
                ...
                uint8_t* outptr = ((uint8_t*)dest) + (stripe * stripe_sz);
