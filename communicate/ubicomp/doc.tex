...
Thanks to the rapid proliferation of connected devices, sensor-generated time series constitute a large and growing portion of the world's data. In order to reduce storage, power, and transmission costs, it is desirable to compress this data, ideally at the point of collection. Unfortunately, existing compression algorithms either perform poorly on sensor-generated time series or violate the memory or latency requirements of many embedded sensing systems.

We introduce a time series compression algorithm that achieves state-of-the-art compression ratios while requiring less than 1KB of memory and as little as eight samples of latency. Moreover, this method is suitable not only for low-power devices collecting data, but also servers storing and querying data; in the latter context, it can decompress at over 3GB/s in a single thread, a significant fraction of memory bandwidth and even faster than many algorithms with lower compression ratios. A key component of our method is a high-speed forecasting algorithm that can be trained online and significantly outperforms alternatives such as delta coding.
