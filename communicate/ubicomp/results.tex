...
All reported timings and throughputs are the best of ten runs. We use the best, rather than average, since this is 1) desirable in the presence of the non-random, purely additive noise present during microbenchmarks\footnote{In the case of compressing contiguous data in main memory, measurement noise stems from other processes running on the same hardware, which can only reduce performance. As an analogy, consider estimating the weight of a truck as its lightest measured weight, rather than its average weight across different loads of unknown cargo. See, e.g., \cite{lemireMicrobenchmark} for further discussion.}, and, consequently, 2) standard practice in microbenchmarking. Moreover, the best values are similar to the averages.
