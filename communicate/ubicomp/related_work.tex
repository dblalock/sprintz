...
\minesp draws upon ideas from time series compression, time series forecasting, integer compression, general-purpose compression, and high-performance computing. We review relevant work in more detail below, but the main distinguishing characteristics of \minesp are its abilities to:
\begin{enumerate}
    \item Bit pack with extremely small block sizes
    \item Bit pack low-bitwidth integers effectively
    \item Exploit correlation between successive samples in \textit{multivariate} time series
...
\end{enumerate}
 ...
 % Because the ideas we use from the last two areas are well-known in the relevant communities, we focus our review on the other areas.
