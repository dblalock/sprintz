...
We introduce a time series compression algorithm that achieves state-of-the-art compression ratios, compression speeds up to 1GB/s, and decompression speeds up to 4GB/s in a single thread. Moreover, because our approach requires $<$1KB of memory, it can be run on low-power sensors at the network edge, offloading computation from the central database and reducing network load. A key component of our method is a low-precision forecasting algorithm that can be trained online and used with any existing compression algorithm.% in addition to our own.
