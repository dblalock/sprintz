...
For Zlib, Zstd, and LZO, we use compression levels of 1 and 9, which correspond to lowest compression but highest speed and high compression but lower speed, respectively.
...
\begin{enumerate}
\itemsep-1mm
    \item \textbf{\texttt{Sprintz -3}}. The full algorithm described in Section~\ref{sec:method}.
    \item \textbf{\texttt{Sprintz -2}}. Like \texttt{Sprintz -3}, but without Huffman coding.
    \item \textbf{\texttt{Sprintz -1}}. Like \texttt{Sprintz -2}, but with delta coding instead of \fire as the forecaster.
\end{enumerate}
...
\begin{figure}[h]
\begin{center}
    \includegraphics[width=\linewidth]{paper/boxplot_ucr}
...
\begin{figure}[h]
\begin{center}
    \includegraphics[width=\linewidth]{paper/cd_diagram_8b}
    \includegraphics[width=\linewidth]{paper/cd_diagram_16b}
...
\begin{center}
    ...
    \caption{\minesp compresses at hundred of MB/s even in the slowest case: the highest-ratio setting with incompressible 8-bit data (left). On lower settings with 16-bit data, it can exceed 1GB/s.}
...
Regarding speed, we have found that the dimensionality and bitwidth almost completely determine \mine's throughput (c.f. Sections~\ref{sec:decomp_speed},~\ref{sec:comp_speed}). The exception is when there are long runs of constants, in which case the decompression speed approaches the speed of \texttt{memcpy} for \texttt{Sprintz -1} or the speed of \fire for \texttt{Sprintz -2} and \texttt{Sprintz -3}.
...
\begin{figure}[h]
\begin{center}
    \includegraphics[width=\linewidth]{paper/tradeoff_success}
    \caption{\minesp achieves excellent compression ratios and speed on smooth time series with many variables.}
...
\begin{figure}[h]
\begin{center}
    \includegraphics[width=\linewidth]{paper/tradeoff_fail}
